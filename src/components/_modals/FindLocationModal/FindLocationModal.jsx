...
export default function FindLocationModal({ setAddressModal, setlocationModal, addressModal,setuserAddressText }) {
    let closeModal = () => {
    ...
    const {
      ...
      const handleCurrentLocation=async()=>{
        ...
        try {
            const response = await fetch(
              `https://maps.googleapis.com/maps/api/geocode/json?latlng=${userDetails.latitude},${userDetails.longitude}&key=AIzaSyCyZYuKJc4YREy3ppZxlnODX_HL7sJlAbk`
            );
      ...
      const renderSuggestions = () =>
        data.map((suggestion) => {
          ...
          return (
            <div key={place_id} onClick={handleSelect(suggestion)} className={styles.locationD}>
    ...
    return (
        <div className={styles.locationMain}>
            <div className={styles.locationBack}>
                ...
                {status === "OK" && <div className={styles.locationE}>{renderSuggestions()}</div>}
            </div>
        </div>
    )
}
