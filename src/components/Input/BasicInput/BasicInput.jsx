...
export default function BasicInput({PlaceHolder,Type,VALUE,DISABLED,id,NAME,updateInputVal,inputVal,required,className,onChange}){
    ...
    return(
        <>
            {
                VALUE?
                <input type={Type?Type:''} placeholder={PlaceHolder?PlaceHolder:''} className={className?`${className} ${styles.basic_input}`:styles.basic_input} value={VALUE?VALUE:''} disabled={DISABLED?DISABLED:false} id={id?id:''} name={NAME?NAME:''} onChange={inputValue} required={required?required:false} />
                :
