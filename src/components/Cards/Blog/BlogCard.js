      ...
      <div className={styles["blogDetails"]}>
        <div className={styles["postedByContainer"]}>
          <div className={styles["userInfo"]}>
            <img loading="lazy" className={styles["userImg"]} src={blog ? blog?.blog_Img?.public_url : ''} alt="userImg" />
            <span className={styles["userName"]}>{blog ? blog?.writer_name : ''}</span>
          </div>
          
          <div className={styles["more"]} onClick={locationBlog}
            >
            <img loading="lazy" src={moreHorizontal} alt="more" className="moreImg"/>
            <div className={shareBlog===blog._id?styles["shareMain"]:styles["d_none"]} onMouseOver={()=>setActiveShare(true)} onMouseOut={()=>setActiveShare(false)} onClick={()=>{setShareModal(true); setShareBlog(null)}} >
              <img loading="lazy" src={activeShare?share_forward_white:share_forward} alt="share" />
              <div>Share blog</div>
            </div>
            <></>
          </div>
